...
class M_datapenyedia extends CI_Model
{
    ...
    // 
    // tambah kbli skdp
    public function tambah_kbli_skdp($data)
    {
        $this->db->insert('tbl_vendor_kbli_skdp', $data);
        return $this->db->affected_rows();
    }
    var $order_skdp =  array('id_vendor_kbli_skdp', 'kode_kbli', 'nama_kbli', 'sts_kbli_skdp', 'id_vendor_kbli_skdp');
    ...
    private function _get_data_query_kbli_skdp($id_vendor)
    {
        ...
        $i = 0;
        foreach ($this->order_skdp as $item) // looping awal
        {
            if ($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                ...
                if (count($this->order_skdp) - 1 == $i)
                    $this->db->group_end();
            }
            $i++;
        }
        if (isset($_POST['order'])) {
            $this->db->order_by($this->order_skdp[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
        } else {
            $this->db->order_by('tbl_vendor_kbli_skdp.id_vendor_kbli_skdp', 'DESC');
        }
    }
    ...
    public function count_filtered_data_kbli_skdp($id_vendor)
    {
        $this->_get_data_query_kbli_skdp($id_vendor); //ambil data dari get yg di atas
        $query = $this->db->get();
        return $query->num_rows();
    ...
    public function count_all_data_kbli_skdp($id_vendor)
    {
        $this->db->select('*');
        $this->db->from('tbl_vendor_kbli_skdp');
        $this->db->join('tbl_kbli', 'tbl_vendor_kbli_skdp.id_kbli = tbl_kbli.id_kbli', 'left');
    ...
    public function get_row_kbli_skdp($id_url_kbli_skdp)
    {
        $this->db->select('*');
        $this->db->from('tbl_vendor_kbli_skdp');
        $this->db->join('tbl_kbli', 'tbl_vendor_kbli_skdp.id_kbli = tbl_kbli.id_kbli', 'left');
        $this->db->join('tbl_kualifikasi_izin', 'tbl_vendor_kbli_skdp.id_kualifikasi_izin = tbl_kualifikasi_izin.id_kualifikasi_izin', 'left');
        $this->db->where('tbl_vendor_kbli_skdp.id_url_kbli_skdp', $id_url_kbli_skdp);
        $query = $this->db->get();
        return $query->row_array();
    }

    public function get_row_kbli_skdp_by_vendor($id_vendor)
    {
        $this->db->select('*');
        $this->db->from('tbl_vendor_kbli_skdp');
        $this->db->join('tbl_kbli', 'tbl_vendor_kbli_skdp.id_kbli = tbl_kbli.id_kbli', 'left');
    ...
    function edit_kbli_skdp($data, $where)
    {
        $this->db->update('tbl_vendor_kbli_skdp', $data, $where);
        return $this->db->affected_rows();
    }

    function hapus_kbli_skdp($where)
    {
        $this->db->delete('tbl_vendor_kbli_skdp', $where);
        return $this->db->affected_rows();
    }
